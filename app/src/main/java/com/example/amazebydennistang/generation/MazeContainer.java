...
public class MazeContainer implements Maze {
	 ...
	 */
	public int[] getNeighborCloserToExit(int x, int y) { //READ!!
		assert isValidPosition(x,y) : "Invalid position";
		// corner case, (x,y) is exit position
		if (floorplan.isExitPosition(x, y))
			return null;
		// find best candidate
		int dnext = getDistanceToExit(x, y) ; //Distance of position to exit
		...
		for (CardinalDirection cd: CardinalDirection.values()) {
			...
			int dn = getDistanceToExit(x+dir[0], y+dir[1]); //Distance of position next to input position from exit
			if (dn < dnext) { //Compares to see 
